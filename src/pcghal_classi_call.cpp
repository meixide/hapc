                                    ...
                                    SEXP verbose_)
{
    ...
    // ---- Risk function: mean(log(1 + exp(-Y * mu))) ----
    ...
    // ---- Gradient function ----
    // grad_j = alpha_j * mean(Y * X[,j] / (1 + exp(Y*mu)))
    auto grad = [&](const VectorXd& alph)->VectorXd {
        ...
        for (int j = 0; j < k; ++j) {
            ...
            for (int i = 0; i < n; ++i) {
                double ymu = y[i] * mu[i];
                // Y[i] * X[i,j] / (1 + exp(Y[i]*mu[i]))
                sum += y[i] * X(i, j) / (1.0 + std::exp(ymu));
            }
            gtmp[j] = alph[j] * sum / static_cast<double>(n);
        }
        return gtmp;
    };
    ...
    if (verbose) {
        Rprintf("Classification mode (logistic loss)\n");
    ...
    for (int iter = 1; iter <= max_iter; ++iter) {
        ...
        if (!valid.empty()) {
            ...
            step = 0.0; // no restriction → don't move
        }
