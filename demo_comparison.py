"""Demo comparing R and Python HAPC implementations."""
...
print("=" * 70)
print("Python HAPC Cross-Validation")
print("=" * 70)

# Run cross-validation
lambdas = np.exp(np.linspace(-6, 0, 20))
print(f"Testing {len(lambdas)} lambda values...")
print(f"Lambda range: [{lambdas.min():.6f}, {lambdas.max():.6f}]")
...
print(f"\nBest lambda: {rescv.best_lambda:.6f}")
print(f"Best MSE: {rescv.mses.min():.6f}")
print(f"Predictions shape: {rescv.predictions.shape}")
...
axes[0].axvline(rescv.best_lambda, color='red', linestyle='--', label=f'Best: {rescv.best_lambda:.6f}')
...
plt.tight_layout()
plt.savefig('/Users/cgmeixide/Projects/hapc/demo_output.png', dpi=150, bbox_inches='tight')
